...
#[repr(u8)]
pub enum SELECT_A {
    #[doc = "1: Unlock Customer Key (access to top half of info0)"]
    CUSTOMER_KEY = 1,
    #[doc = "0: Lock Control should be set to NONE when not in use."]
    NONE = 0,
}
...
impl<'a> SELECT_W<'a> {
    ...
    #[doc = "Unlock Customer Key (access to top half of info0)"]
    #[inline(always)]
    pub fn customer_key(self) -> &'a mut W {
        self.variant(SELECT_A::CUSTOMER_KEY)
    }
    #[doc = "Lock Control should be set to NONE when not in use."]
    #[inline(always)]
